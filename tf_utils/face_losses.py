# -*- coding:utf-8 -*-
from __future__ import absolute_import

"""
code ref: https://github.com/auroua/InsightFace_TF

"""
import math

import tensorflow as tf

_np_zero = 1e-15


def arcface_loss_layer(embeddings, labels, out_num: int, w_init=tf.contrib.layers.xavier_initializer(uniform=False),
                       s: float = 64., m: float = 0.5, scope: str = "arcface_loss"):
    """
    :param embeddings: the input embedding vectors
    :param labels:  the input labels, the shape should be eg: (batch_size, 1)
    :param s: scalar value default is 64
    :param out_num: output class num
    :param w_init: weight init function
    :param m: the margin value, default is 0.5
    :param scope: scope name
    :return: the final calculated output, this output is send into the tf.nn.softmax directly
    """
    cos_m = math.cos(m)
    sin_m = math.sin(m)
    threshold = math.cos(math.pi - m)

    with tf.variable_scope(scope):
        # 1. norm X
        embeddings = tf.nn.l2_normalize(embeddings, axis=1, name='norm_embedding')  # axis = 1, self

        # 2. norm W
        weights = tf.get_variable(name='embedding_weights', shape=(embeddings.get_shape().as_list()[-1], out_num),
                                  initializer=w_init, dtype=tf.float32)
        weights = tf.nn.l2_normalize(weights, axis=0, name='norm_weights')  # axis = 0, batch

        # 3. cos(theta+m)
        # cos(theta+m) = cos(theta)cos(m) - sin(theta)sin(m)
        # sin(theta) = sqrt(1-cos(theta)*cos(theta))
        cos_theta = tf.matmul(embeddings, weights, name='cos_theta')
        sin_theta = tf.sqrt(tf.subtract(1., tf.square(cos_theta)), name='sin_theta')
        cos_theta_m = tf.subtract(tf.multiply(cos_theta, cos_m), tf.multiply(sin_theta, sin_m), name='cos_theta_m')
        cos_theta_m = s * cos_theta_m

        # 4. cos(theta+m) 单调性
        # this condition controls the theta+m should in range [0, pi]
        #      0<=theta+m<=pi
        #     -m<=theta<=pi-m
        # 若s*cos(theta+m)小于0，使用s*cos(theta)-s*m*sin(m)
        cond = tf.cast(tf.nn.relu(cos_theta - threshold, name='if_else'), dtype=tf.bool)  # from source code
        cos_theta_m_value = tf.where(cond, cos_theta_m, s * (cos_theta - m * sin_m))

        mask = tf.one_hot(labels, depth=out_num, name='one_hot_mask')
        inv_mask = tf.subtract(1., mask, name='inverse_mask')

        logit = tf.add(tf.multiply(s * cos_theta, inv_mask), tf.multiply(cos_theta_m_value, mask), name='logit')

        # net = tf.nn.softmax(logits)
        # loss = tf.reduce_mean(-tf.reduce_sum(mask * tf.log(net), reduction_indices=[1]))
        #
        # return logits, loss

        return logit


def cosineface_losses(embedding, labels, out_num, w_init=None, s=30., m=0.4):
    """
    :param embedding: the input embedding vectors
    :param labels:  the input labels, the shape should be eg: (batch_size, 1)
    :param s: scalar value, default is 30
    :param out_num: output class num
    :param w_init: weight init function
    :param m: the margin value, default is 0.4
    :return: the final calculated output, this output is send into the tf.nn.softmax directly
    """
    with tf.variable_scope('cosineface_loss'):
        # inputs and weights norm
        embedding_norm = tf.norm(embedding, axis=1, keep_dims=True)
        embedding = tf.div(embedding, embedding_norm, name='norm_embedding')
        weights = tf.get_variable(name='embedding_weights', shape=(embedding.get_shape().as_list()[-1], out_num),
                                  initializer=w_init, dtype=tf.float32)
        weights_norm = tf.norm(weights, axis=0, keep_dims=True)
        weights = tf.div(weights, weights_norm, name='norm_weights')
        # cos_theta - m
        cos_t = tf.matmul(embedding, weights, name='cos_t')
        cos_t_m = tf.subtract(cos_t, m, name='cos_t_m')

        mask = tf.one_hot(labels, depth=out_num, name='one_hot_mask')
        inv_mask = tf.subtract(1., mask, name='inverse_mask')

        output = tf.add(s * tf.multiply(cos_t, inv_mask), s * tf.multiply(cos_t_m, mask), name='cosineface_loss_output')
    return output


def combine_loss_val(embedding, labels, w_init, out_num, margin_a, margin_m, margin_b, s):
    """
    This code is contributed by RogerLo. Thanks for you contribution.

    :param embedding: the input embedding vectors
    :param labels:  the input labels, the shape should be eg: (batch_size, 1)
    :param s: scalar value default is 64
    :param w_init: weight init function
    :param out_num: output class num
    :param m: the margin value, default is 0.5
    :return: the final cacualted output, this output is send into the tf.nn.softmax directly
    """
    weights = tf.get_variable(name='embedding_weights', shape=(embedding.get_shape().as_list()[-1], out_num),
                              initializer=w_init, dtype=tf.float32)
    weights_unit = tf.nn.l2_normalize(weights, axis=0)
    embedding_unit = tf.nn.l2_normalize(embedding, axis=1)
    cos_t = tf.matmul(embedding_unit, weights_unit)
    ordinal = tf.constant(list(range(0, embedding.get_shape().as_list()[0])), tf.int64)
    ordinal_y = tf.stack([ordinal, labels], axis=1)
    zy = cos_t * s
    sel_cos_t = tf.gather_nd(zy, ordinal_y)
    if margin_a != 1.0 or margin_m != 0.0 or margin_b != 0.0:
        if margin_a == 1.0 and margin_m == 0.0:
            s_m = s * margin_b
            new_zy = sel_cos_t - s_m
        else:
            cos_value = sel_cos_t / s
            t = tf.acos(cos_value)
            if margin_a != 1.0:
                t = t * margin_a
            if margin_m > 0.0:
                t = t + margin_m
            body = tf.cos(t)
            if margin_b > 0.0:
                body = body - margin_b
            new_zy = body * s
    updated_logits = tf.add(zy, tf.scatter_nd(ordinal_y, tf.subtract(new_zy, sel_cos_t), zy.get_shape()))
    loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=updated_logits))
    predict_cls = tf.argmax(updated_logits, 1)
    accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.cast(predict_cls, tf.int64), tf.cast(labels, tf.int64)), 'float'))
    predict_cls_s = tf.argmax(zy, 1)
    accuracy_s = tf.reduce_mean(tf.cast(tf.equal(tf.cast(predict_cls_s, tf.int64), tf.cast(labels, tf.int64)), 'float'))
    return zy, loss, accuracy, accuracy_s, predict_cls_s
